...
int PhanLoai::load(AAssetManager *mgr, const char *modelType) {
    model.clear();
    ...
    char paramPath[255];
    char modelPath[255];
    sprintf(paramPath, "model_%s.param", modelType);
    sprintf(modelPath, "model_%s.bin", modelType);
    model.load_param(mgr, paramPath);
    model.load_model(mgr, modelPath);
    return 0;
}

int PhanLoai::predict(cv::Mat src, std::vector<float> &result) {
...
int PhanLoai::predictPath(const std::string &imagePath, std::vector<float> &result) {
