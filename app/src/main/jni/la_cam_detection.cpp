...
la_cam_detection::la_cam_detection() {
    blob_pool_allocator.set_size_compare_ratio(0.f);
    workspace_pool_allocator.set_size_compare_ratio(0.f);
}

int la_cam_detection::load(AAssetManager *mgr) {
    yolo.clear();
...
//#if NCNN_VULKAN
//	la_cam_detection.opt.use_vulkan_compute = use_gpu;
//#endif
    ...
    char parampath[256];
    char modelpath[256];
    sprintf(parampath, "la_cam.param");
    sprintf(modelpath, "la_cam.bin");
...
int
la_cam_detection::detect(const cv::Mat &rgb, std::vector<Object> &objects, float prob_threshold,
                         float nms_threshold) {
    int width = rgb.cols;
    int height = rgb.rows;
...
int la_cam_detection::predictPath(const std::string &imagePath, std::vector<Object> &objects,
