...
int cay_benh::load(AAssetManager *mgr, const char *model_name) {
    model.clear();
    ...
    sprintf(modelPath, "%s.bin", model_name);
    model.load_param(mgr, paramPath);
    model.load_model(mgr, modelPath);
    return 0;
}

int cay_benh::predict(cv::Mat src, std::vector<float> &result) {
...
int cay_benh::predictPath(const std::string &imagePath, std::vector<float> &result) {
