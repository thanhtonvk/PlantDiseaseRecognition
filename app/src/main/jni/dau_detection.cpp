...
DauDetection::DauDetection() {

}

int DauDetection::load(AAssetManager *mgr, const char *modelType) {
    model.clear();
    ...
    char paramPath[255];
    char modelPath[255];
    sprintf(paramPath, "model_cay_dau.param");
    sprintf(modelPath, "model_cay_dau.bin");
    model.load_param(mgr, paramPath);
    model.load_model(mgr, modelPath);
    return 0;
}

int DauDetection::predict(cv::Mat src, std::vector<float> &result) {
...
int DauDetection::predictPath(const std::string &imagePath, std::vector<float> &result) {
