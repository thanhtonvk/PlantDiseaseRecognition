...
BoDetection::BoDetection() {

}

int BoDetection::load(AAssetManager *mgr) {
    model.clear();
    ...
    char paramPath[255];
    char modelPath[255];
    sprintf(paramPath, "bo.param");
    sprintf(modelPath, "bo.bin");
    model.load_param(mgr, paramPath);
    model.load_model(mgr, modelPath);
    return 0;
}

int BoDetection::predict(cv::Mat src, std::vector<float> &result) {
    ...
    in_net.substract_mean_normalize(0, norm);
...
int BoDetection::predictPath(const std::string &imagePath, std::vector<float> &result) {
