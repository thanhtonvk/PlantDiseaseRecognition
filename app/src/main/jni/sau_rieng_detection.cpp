...
sau_rieng_detection::sau_rieng_detection() {
    blob_pool_allocator.set_size_compare_ratio(0.f);
    workspace_pool_allocator.set_size_compare_ratio(0.f);
}

int sau_rieng_detection::load(AAssetManager *mgr) {
    yolo.clear();
...
//#if NCNN_VULKAN
//	sau_rieng_detection.opt.use_vulkan_compute = use_gpu;
//#endif
    ...
    char parampath[256];
    char modelpath[256];
    sprintf(parampath, "sau_rieng.param");
    sprintf(modelpath, "sau_rieng.bin");
...
int
sau_rieng_detection::detect(const cv::Mat &rgb, std::vector<Object> &objects, float prob_threshold,
                            float nms_threshold) {
    int width = rgb.cols;
    int height = rgb.rows;
...
int sau_rieng_detection::predictPath(const std::string &imagePath, std::vector<Object> &objects,
